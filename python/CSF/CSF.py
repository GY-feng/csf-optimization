# ======================================================================================
# CSF.py - SWIG 生成的 Python 包装器
#
# 这个文件是自动生成的，不需要修改。
# 它把 C++ 的 CSF 类包装成 Python 可调用的对象，
# 包括：
#   1. SwigPyIterator - 迭代器类
#   2. VecInt, VecFloat, VecDouble - 向量类包装
#   3. Params - 参数类包装
#   4. CSF - 主类包装
#
# 使用方法：
#   import CSF
#   csf = CSF.CSF()
#   csf.setPointCloud(points)  # 传入 numpy 数组
...
class Params(object):
    ...
    useSoA = property(_CSF.Params_useSoA_get, _CSF.Params_useSoA_set)
...
class CSF(object):
    ...
    def getLastProfileJson(self):
        return _CSF.CSF_getLastProfileJson(self)

    def configureOptimization(self, memoryOptimized, deterministicSoA, gpuEnabled, gpuSimulation, gpuClassification, gpuRasterization, gpuDeviceId):
        return _CSF.CSF_configureOptimization(self, memoryOptimized, deterministicSoA, gpuEnabled, gpuSimulation, gpuClassification, gpuRasterization, gpuDeviceId)
